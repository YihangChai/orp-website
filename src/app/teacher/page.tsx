...
export default function TeacherPage() {
  ...
  useEffect(() => {
    async function loadCurrentTeacher() {
      ...
      const activeTeacher: TeacherRow = {
        id: teacher.id,
        name: teacher.name,
        email: teacher.email,
      ...
      await fetchTeacherData(teacher.id);
    }
    ...
    return (
      <main className="min-h-screen bg-[#f6f5e9] px-6 py-10 text-stone-800">
        <section className="mx-auto max-w-4xl rounded-[2rem] border border-emerald-100 bg-white p-7 shadow-sm">
          <p className="text-sm text-stone-600">正在确认小老师身份...</p>
        </section>
      </main>
    );
  }
      ...
      <main className="min-h-screen bg-[#f6f5e9] px-6 py-10 text-stone-800">
        <section className="mx-auto max-w-4xl rounded-[2rem] border border-emerald-100 bg-white p-7 shadow-sm">
          ...
          <p className="mt-3 text-sm leading-7 text-stone-600">
            小老师需要使用邮箱和密码登录后，才能查看自己的班级、学生、教学目标和课程记录。
          </p>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <h2 className="mt-1 text-2xl font-bold text-emerald-950">
                    {currentTeacher?.name || teacherSession.teacherName}
                  </h2>
